...
2. 启动后台服务和托盘/菜单栏控制器。
3. 打开托盘菜单中的管理页面：Windows 服务监听 `0.0.0.0:59188`，可用 `http://<主机IP>:59188` 访问，
   本机也可用 `http://127.0.0.1:59188`；macOS 与 Linux 安装包固定使用回环地址 `http://127.0.0.1:59188`。
...
Linux、Windows 和 macOS 桌面安装包固定使用网页初始化：启动服务后在“首次设置管理员密码”页面
输入并确认不少于 8 个字符的密码，系统会创建 `admin` 并自动登录。打开方式按平台区分：Windows 服务
监听 `0.0.0.0:59188`，可从局域网其他主机用 `http://<主机IP>:59188` 访问，本机同样可用
`http://127.0.0.1:59188`；macOS 和 Linux 安装包保持回环地址 `http://127.0.0.1:59188`，不对外监听。
将 Windows 管理端口开放到局域网时，必须用 Windows 防火墙或等价的网络 ACL 限制来源，不要暴露到公网。
源码运行的默认地址是 `:59188`，访问地址取决于传入的 `-addr`；
