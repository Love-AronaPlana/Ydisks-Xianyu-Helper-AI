...
if [ -n "${MACOS_SIGNING_IDENTITY:-}" ]; then
  ...
  find "$APP/Contents/Resources/playwright-runtime" -type d -name '*.app' -print | while IFS= read -r nested_app; do
    if [ -n "${MACOS_SIGNING_KEYCHAIN:-}" ]; then
      codesign --force --deep --sign "$MACOS_SIGNING_IDENTITY" \
        --keychain "$MACOS_SIGNING_KEYCHAIN" --timestamp=none "$nested_app"
    else
      codesign --force --deep --sign "$MACOS_SIGNING_IDENTITY" \
        --timestamp=none "$nested_app"
    fi
  done
