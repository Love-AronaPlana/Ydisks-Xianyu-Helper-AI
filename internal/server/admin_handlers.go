...
func (s *Server) adminListUsers(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		...
		s.Store.DB.QueryRowContext(r.Context(),
			`SELECT COUNT(*) FROM cookies WHERE user_id=?`, id).Scan(&cookieCount)
...
func (s *Server) adminStats(w http.ResponseWriter, r *http.Request) {
	...
	s.Store.DB.QueryRowContext(r.Context(), `SELECT COUNT(*) FROM users`).Scan(&totalUsers)
	s.Store.DB.QueryRowContext(r.Context(), `SELECT COUNT(*) FROM cookies`).Scan(&totalCookies)
	s.Store.DB.QueryRowContext(r.Context(), `SELECT COUNT(*) FROM cards`).Scan(&totalCards)
	s.Store.DB.QueryRowContext(r.Context(), `SELECT COUNT(*) FROM orders`).Scan(&totalOrders)
	s.Store.DB.QueryRowContext(r.Context(), `SELECT COUNT(*) FROM keywords`).Scan(&totalKeywords)
