...
func TestKeywordHandlersCoverCompatibilityAndErrorMappings(t *testing.T) {
	...
	successCases := []struct {
		...
		{name: "add item multi", hand: server.addKeywordWithItemID, req: requestWithKeywordParams(http.MethodPost, "/keywords-with-item/cid", `{"keyword":"hello","reply":"world","item_ids":["item-1","item-2"],"type":"text"}`, params)},
		...
		{name: "update multi", hand: server.updateKeywordByID, req: requestWithKeywordParams(http.MethodPut, "/keywords-with-type/cid/7", `{"keyword":"hello","reply":"changed","type":"text","item_ids":["item-1","item-2"]}`, params)},
	...
	errorCases := []struct {
		...
		{name: "update multi", hand: server.updateKeywordByID, req: requestWithKeywordParams(http.MethodPut, "/keywords-with-type/cid/7", `{"keyword":"k","reply":"r","item_ids":["item-2"]}`, params)},
		{name: "add item multi", hand: server.addKeywordWithItemID, req: requestWithKeywordParams(http.MethodPost, "/keywords-with-item/cid", `{"keyword":"hello","reply":"world","item_ids":["item-1"]}`, params)},
		{name: "delete id", hand: server.deleteKeywordByID, req: requestWithKeywordParams(http.MethodDelete, "/keywords-with-type/cid/7", "", params)},
		{name: "delete index", hand: server.deleteKeyword, req: requestWithKeywordParams(http.MethodDelete, "/keywords/cid/0", "", params)},
