...
func newApplicationServices(server *Server) *applicationServices {
	...
	return &applicationServices{
		orders:        &orderApplicationService{server: newServerOrderRuntime(server), repository: orderRepository, list: orderapp.NewListService(orderRepository), detail: orderapp.NewDetailService(orderRepository), refreshJobs: newStoreOrderRefreshJobRepository(server.Store)},
