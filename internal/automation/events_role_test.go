...
func roleEventFixture(t *testing.T, trigger, source, role string) map[string]any {
	...
	raw := mustMap(t, fmt.Sprintf(`{"1":{"2":"60000000001@goofish","7":1,"10":{
		"reminderContent":%q,"senderUserId":"cid",
		"reminderUrl":"fleamarket://message_chat?itemId=1000000000002&peerUserId=cid&sid=60000000001",
		"extJson":%q}}}`, text, fmt.Sprintf(`{"updateKey":"60000000001:3310145690545023994:10:%s:26","contentType":"26"}`, businessKey)))
	if role == "" {
		return raw
	}
	...
	case "reminderUrl":
		notice["reminderUrl"] = link + "&itemId=1000000000002"
	case "contentCard":
			...
			for _, role := range []string{"buyer", "seller", ""} {
				...
				t.Run(trigger+"/"+source+"/"+role, func(t *testing.T) {
					...
					if task.TriggerType != trigger || task.AccountID != "cid" || task.Source != "ws" ||
						task.OrderID != "3310145690545023994" || task.ChatID != "60000000001" ||
						task.ItemID != "1000000000002" || task.BuyerID != "cid" {
