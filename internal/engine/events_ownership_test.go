	...
	for _, roles := range [][]string{
		...
		t.Run(fmt.Sprint(roles), func(t *testing.T) {
			...
			for step, role := range roles {
				...
				if decodeErr := json.Unmarshal([]byte(fmt.Sprintf(`{"1":{"2":"60000000001@goofish","10":{
					"reminderContent":"[我完成了评价]","redReminder":"有新交易评价","senderUserId":"platform-buyer",
					"reminderUrl":"fleamarket://order_detail?id=3310145690545023994&role=%s&itemId=1000000000002",
					"extJson":"{\"updateKey\":\"60000000001:3310145690545023994:10:BUYER_RATE_SELLER:26\",\"contentType\":\"26\"}"}}}`, role)), &raw); decodeErr != nil {
					t.Fatal(decodeErr)
				}
				...
				} else if order.CookieID != "cid" || order.BuyerID != "platform-buyer" || order.ItemID != "1000000000002" || order.ChatID != "60000000001" || order.BuyerReviewedAt == "" {
