...
// TestPublishBatches_CreateGetRows 负责Test发布批次列表CreateGetRows相关处理。
...
// TestPublishBatchesListAndRecoverInterrupted 负责Test发布批次列表ListAndRecoverInterrupted相关处理。
...
func TestPublishBatches_PendingRowsAndStatus(t *testing.T) {
	...
	// pendingAfter 保存pendingAfter，供当前处理流程使用
...
// TestPublishBatches_ClaimsAreAtomicAndLeaseCanBeRecovered 负责Test发布批次列表ClaimsAreAtomicAndLeaseCanBeRecovered相关处理。
...
// TestPublishBatches_ResetFailedKeepsValidationFailures 负责Test发布批次列表Reset失败KeepsValidationFailures相关处理。
...
// TestPublishBatchLeaseTakeoverQuarantinesRemoteStartedRow 负责Test发布批次LeaseTakeoverQuarantinesRemoteStartedRow相关处理。
func TestPublishBatchLeaseTakeoverQuarantinesRemoteStartedRow(t *testing.T) {
	...
	if // pending、err 保存pending、err，供当前处理流程使用
...
// TestPublishBatchCancelPreservesRemoteCheckpointBeforeFinalizing 负责Test发布批次取消PreservesRemoteCheckpointBeforeFinalizing相关处理。
func TestPublishBatchCancelPreservesRemoteCheckpointBeforeFinalizing(t *testing.T) {
	...
	// token、running、err 保存token、running、err，供当前处理流程使用
...
// TestFinalizeBatchRejectsUnfinishedRows 负责TestFinalize批次RejectsUnfinishedRows相关处理。
func TestFinalizeBatchRejectsUnfinishedRows(t *testing.T) {
	...
	// status、finalized、err 保存status、finalized、err，供当前处理流程使用
...
// TestExpiredCancelingBatchIsRecoverableAndFinalizedSafely 负责TestExpiredCanceling批次IsRecoverableAndFinalizedSafely相关处理。
func TestExpiredCancelingBatchIsRecoverableAndFinalizedSafely(t *testing.T) {
	...
	if // running、err 保存running、err，供当前处理流程使用
...
// TestPublishBatchFinalizersFenceStaleWorker 负责Test发布批次FinalizersFenceStale工作器相关处理。
