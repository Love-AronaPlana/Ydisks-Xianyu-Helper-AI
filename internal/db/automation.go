...
// ErrAutomationRunActive 保存Err自动化运行Active，供当前处理流程使用
...
// AutomationRules 保存自动化规则列表，供当前处理流程使用
...
// AutomationRule 保存自动化规则，供当前处理流程使用
...
// DeferredAutomationTask 保存Deferred自动化任务，供当前处理流程使用
...
// ErrDeferredTaskLeaseLost 保存ErrDeferred任务LeaseLost，供当前处理流程使用
var ErrDeferredTaskLeaseLost = errors.New("延迟自动化任务租约已失效")

// AutomationRunIssue 保存自动化运行问题，供当前处理流程使用
...
// DeferredAutomationIssue 保存Deferred自动化问题，供当前处理流程使用
...
// automationRuleWhere 负责自动化规则Where相关处理。
...
func (a *AutomationRules) ListPageForUser(ctx context.Context, f AutomationRuleListFilter) ([]AutomationRule, int, error) {
	// whereSQL、args 保存whereSQL、args，供当前处理流程使用
...
// CountByTriggerForUser 负责数量ByTriggerFor用户相关处理。
...
// highestPriorityRule 负责highest优先级规则相关处理。
...
// AbortRunAction 负责Abort运行动作相关处理。
...
// RenewRunLease 负责Renew运行Lease相关处理。
...
// QuarantineRun 负责Quarantine运行相关处理。
...
// QuarantineRunResult 负责Quarantine运行结果相关处理。
...
// ClaimRecoveryRun 负责ClaimRecovery运行相关处理。
...
func (a *AutomationRules) FinishRun(ctx context.Context, id int64, attempt int, status string, sentCount int, errMsg string) error {
	// nextRetryAt 保存next重试At，供当前处理流程使用
	nextRetryAt := int64(0)
...
// requireAutomationRunOwner 负责require自动化运行所有者相关处理。
...
// DueRecoveryRuns 负责DueRecovery运行记录相关处理。
...
// RecoverDefinitelyUnsentReviewRuns 负责RecoverDefinitelyUnsentReview运行记录相关处理。
...
func (a *AutomationRules) ListIssues(ctx context.Context, userID int64) ([]AutomationRunIssue, []DeferredAutomationIssue, error) {
	// runRows、err 保存运行Rows、err，供当前处理流程使用
	...
	// runs 保存运行记录，供当前处理流程使用
	runs := []AutomationRunIssue{}
	for runRows.Next() {
		// issue 保存问题，供当前处理流程使用
		var issue AutomationRunIssue
		// rawEventJSON 保存原始EventJSON，供当前处理流程使用
		var rawEventJSON string
		// actionStarted、ruleEnabled 保存动作Started、rule启用状态，供当前处理流程使用
	...
	// taskRows、err 保存任务Rows、err，供当前处理流程使用
...
func (a *AutomationRules) ResolveRunIssue(ctx context.Context, userID, runID int64, resolution string) error {
	// rawEventJSON、errorMessage 保存原始EventJSON、error消息，供当前处理流程使用
	var rawEventJSON, errorMessage string
	// actionStarted、ruleEnabled、sentCount 保存动作Started、ruleEnabled、sent数量，供当前处理流程使用
...
// automationIssuePolicy 负责自动化问题Policy相关处理。
...
// containsString 负责containsString相关处理。
...
// ClaimDueDeferredTasks 负责ClaimDueDeferred任务列表相关处理。
...
// FinishDeferredTask 负责FinishDeferred任务相关处理。
...
// deferredRetryDelay 负责deferred重试延迟相关处理。
...
// RenewDeferredTaskLease 负责RenewDeferred任务Lease相关处理。
...
// requireDeferredTaskOwner 负责requireDeferred任务所有者相关处理。
...
// createAutomationRuleTx 负责create自动化规则Tx相关处理。
...
// insertAutomationActionTx 负责insert自动化动作Tx相关处理。
