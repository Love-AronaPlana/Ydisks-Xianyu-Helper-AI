...
本 Wiki 面向管理后台操作人员。先完成“部署与数据库”和“账号接入”，再配置库存、商品、自动化与 AI。
...
6. 在“自动化规则”建立付款发货、评价赠品或求评价规则。
7. 可选：在“系统与 AI”配置模型，再为每个账号启用 AI。
8. 建立通知渠道并绑定到账号；用测试通知验证。
9. 用小额测试订单验证“卡密发送 → 确认发货”全过程后，再开放正式商品。
