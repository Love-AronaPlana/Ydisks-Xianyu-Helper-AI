...
12. 用小额测试订单验证卡密发送和库存扣减；如果账号启用了“自动确认发货”，再确认平台状态随后变为已发货后，才开放正式商品。

桌面安装包的管理地址按平台区分。Windows 服务监听 `0.0.0.0:59188`，局域网内其他主机可用
`http://<主机IP>:59188` 访问，本机也可用 `http://127.0.0.1:59188`；该端口必须由 Windows 防火墙或
等价网络 ACL 限制来源，不得暴露到公网。macOS 和 Linux 安装包保持回环地址
`http://127.0.0.1:59188`。Windows 和 macOS 安装包自带后台服务、托盘/菜单栏
