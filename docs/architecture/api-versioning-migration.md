...
| 账号 | `/cookies`、`/cookies/details`、`/cookies/runtime-status`、`/cookies/{cid}/...` | `/api/v1/accounts`、`/api/v1/accounts/details`、`/api/v1/accounts/runtime-status`、`/api/v1/accounts/{cid}` | React 摘要、详情、运行状态和启停状态调用已迁移；旧路径保留并由同一 handler 提供 |
...
- 账号摘要、非敏感详情、运行状态和启停状态已由 React 请求层迁移到 `/api/v1/accounts...`；Go 契约测试确认账号详情不泄露 Cookie/密码，且旧详情入口仍可用。
