...
过渡期已有依赖可以在对应纵向切片完成前保留，但不得新增新的直接调用点。
...
1. 先禁止新增不允许依赖；
2. 每迁移一个业务切片，删除对应例外；
3. 最终使用 `go list` 或自定义 AST 检查完整依赖；
4. 前端使用 TypeScript/ESLint import 限制；
5. 架构例外必须记录负责人、原因和删除阶段；
6. 禁止为了让门禁通过而把依赖藏进反射、动态 import 或 service locator。
