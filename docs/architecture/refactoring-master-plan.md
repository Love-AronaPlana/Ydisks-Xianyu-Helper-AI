...
| 2. 类型化客户端与登录账号主链路 | 当前阶段 | 迁移 session、system、accounts、QR 风控和永久关闭的 password-login。 |
| 3. 查询、聊天与订单主链路 | 待执行 | 迁移 dashboard、admin、chat、orders、刷新任务和 WebSocket 消息。 |
...
阶段最终验收失败时状态仍为当前阶段，修复后重跑完整命令；只有全部命令成功后才一次更新状态、证据和下一阶段入口并创建最终中文提交。不得扩大白名单、baseline、忽略路径或 warning-only 旁路；后续缺陷修复保持全部已启用门禁永久有效。
