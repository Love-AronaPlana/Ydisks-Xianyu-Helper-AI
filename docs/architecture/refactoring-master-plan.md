...
- 2026-09-23（提示词全部取消长度上限，本地未发布）：在上一轮去掉完全模式提示词上限之后，继续清掉商品级 AI 提示词 `prompt` 的 8000 字符上限（`internal/application/items/ai_prompt.go` 的校验分支与 `api/openapi.yaml` 两处 `maxLength`），前端契约类型重新生成；自定义提示词本就无上限。新增 `internal/application/items/ai_prompt_test.go` 覆盖 20 万字符提示词可写入且不截断、非法策略仍被拒绝。至此三类提示词（完全模式、自定义、商品级）都不再限制长度，可用长度只由模型上下文窗口决定。

- 2026-09-23（AI 上下文扩到 30 轮并纳入人工客服回复，本地未发布）：按用户要求把 AI 上下文从最近 10 条消息扩到 30 轮问答，并让人工客服的回复进入上下文。一，`ConversationHistory` 的硬上限由 20 提升到 200，默认值 60 条（= 30 轮问答），引擎以 `aiConversationRows = 60` 请求；二，新增 `AIReply.AddAIConversationMessage` 写入单条会话消息，聊天应用新增 `AIConversationRecorder` 端口（由 `chatRepository` 实现并加编译期断言），界面人工发送的文字与图片在发送成功后以 `role=assistant`、`intent=human` 写入上下文，内容带 `engine.HumanReplyContentPrefix`（【人工客服】）前缀，AI/自动化回复不重复写入；`OutgoingInput`/`ImageInput` 增加 `HumanReply` 标记，仅由聊天界面处理器置真；三，系统提示词追加一行说明该前缀含义，避免模型重复或反驳真人已经给出的答复。写入失败只影响上下文完整性，不影响已发出的消息。验证：`go build ./...`、`go vet ./...`、`internal/engine`+`internal/db`+`internal/application/chat`+`internal/application/items`+`internal/server` 测试（仅既有 Windows 平台断言 `TestPublishImageAndSheetHelpersCoverPathAndHeaderBranches` 失败）、`go run ./tools/commentlint`、`go run ./tools/architecturecheck` 均通过。未修改冻结 CAPTCHA、六阶段状态或注释基线。

- 2026-09-23（完全模式提示词取消长度上限，本地未发布）：按用户要求去掉提示词长度限制。删除应用层 `MaxAIReplyPromptLength` 常量、`ErrAIReplyPromptTooLong` 错误与校验分支、HTTP 层对应的 400 分支，以及前端输入框的 `maxLength`；`api/openapi.yaml` 的两处 `ai_full_prompt` 同步移除 `maxLength` 并重新生成前端契约类型。新增迁移 `00057_ai_prompt_capacity`（三方言编号一致）：MySQL 把 `ai_full_prompt` 与 `custom_prompts` 由 TEXT（64KB）改为 LONGTEXT，SQLite/PostgreSQL 的 TEXT 本就不限长，保留同名空迁移维持编号一致；`migrate_test` 的最终版本期望更新为 57。新增用例 `TestUpsertAIReplyAcceptsUnlimitedPromptLength` 验证 12 万字符提示词可写入并原样读回。**注意**：可用长度实际受模型上下文窗口限制，提示词过大时模型调用会失败并在日志中记录“AI 调用失败”。验证：`go build ./...`、`internal/application/settings`+`internal/db`+`internal/server` 测试（仅既有 Windows 平台断言 `TestPublishImageAndSheetHelpersCoverPathAndHeaderBranches` 失败）、`go run ./tools/commentlint`、`go run ./tools/architecturecheck`、`npm run api:check`、`npm run typecheck`、`npm run comments:check`、账户特性 68 项前端测试与嵌入前端重建均通过。未修改冻结 CAPTCHA、六阶段状态或注释基线。

- 2026-09-23（隐私数据清理与历史重写，本地未发布）：按用户要求清理已上传的真实数据。删除 `docs/img/preview_account.png` 与 `preview_im.png`（含真实闲鱼账号 ID、买家昵称头像、买家私密备注与聊天记录），README 与站点首页去掉对应引用；`order-sync-ownership-fix-plan.md`、`批量铺货使用文档.md`、`internal/chat/service_test.go`、`internal/automation/events_test.go`、`internal/engine/reply_extra_test.go` 中的真实账号/订单/商品/会话标识全部替换为占位标识（断言不变）；`AGENTS.md` 与 `frontend/README.md` 的 macOS 本机绝对路径改为可移植路径。随后用 `git filter-repo` 重写全部历史（移除两条图片路径 + 按规则替换真实标识），515 个提交全部重建，本地与镜像 `ai` 已强推对齐；用全新克隆独立复核：历史中不再存在这两张图片与任何真实标识。**注意**：上游 `Christ9038/Ydisks-Xianyu-Helper` 的历史仍是旧版本（含上述截图与真实标识），其清理需另行确认；重写后本项目与上游的提交 SHA 不再一一对应，后续“对齐上游”需要按内容比对。未修改冻结 CAPTCHA、六阶段状态或注释基线。

- 2026-09-23（人工接管买家标识归一与到期清理，本地未发布）：排查“买家说人工后 AI 到点没有恢复”的反馈，先把接管链路的每一层都用真实 SQLite 验证一遍：`ActivateHumanHandoff` 的 UPSERT 语义（首次写入生效、更晚窗口覆盖、更早窗口不缩短、历史过期记录可被新窗口覆盖）此前没有数据库级测试，现补 `TestActivateHumanHandoffUpsertSemantics` 固化；到期判定为 Unix 秒严格大于比较，边界相等即放行。真正的缺陷是隔离键不一致：入站发送者标识有时带 `@goofish` 后缀、有时不带，而引擎按原始值写入与查询、会话页面按 `chat_sessions.buyer_id` 原始值读写，同一个买家可能被拆成两个接管目标，出现接管不生效或无法提前结束。现新增 `engine.NormalizeHumanHandoffBuyerID`（去空白与协议后缀），引擎激活与查询、`chatRepository.FindSessionBuyer` 全部使用归一键，并补 `var _ chatapp.HumanHandoffRepository = chatRepository{}` 编译期断言（此前缺少该断言，接口不匹配只在运行期暴露）。同时把手工接管的状态查询改为读取截止时间：到期后删除过期记录并输出一次“人工接管已到期，恢复 AI 回复”日志，激活时输出“人工接管已生效”与截止时刻，便于运营在日志中直接确认 AI 何时恢复；所有日志只记录买家标识与时间，不记录消息正文。验证：`go build ./...`、`go vet ./...`、`go run ./tools/architecturecheck`、`go run ./tools/commentlint`、`internal/engine`+`internal/adapter`+`internal/db`+`internal/application/*`+`internal/server` 全量测试（仅既有 Windows 平台断言 `TestPublishImageAndSheetHelpersCoverPathAndHeaderBranches` 失败）均通过。重新构建 `dist/windows/xianyu-server.exe`（SHA-256 `C8533D615F1DFFB3025069DCF6E5DF6710D6116A61E189919909F773A2E93DB7`）与安装包（SHA-256 `A9F7B75D78EDEF16577BAA71FD5226BC1149EBB726D377646D5FE2A139E4CA5C`），新二进制以临时数据库启动 `/health` 返回 200。未修改 HTTP 契约、数据库 schema、冻结 CAPTCHA、六阶段状态或注释基线；未调用真实账号或平台交易接口。

- 2026-09-23（镜像仓库发布与 Wiki 同步修复，本地未发布）：按用户要求把项目推送到 GitHub 镜像仓库 `Love-AronaPlana/Ydisks-Xianyu-Helper-AI`，本地新增远程名 `ai`，`origin` 保持 `Christ9038/Ydisks-Xianyu-Helper` 不变。`sync-wiki.yml` 原先把上游仓库写死在克隆地址里：在镜像仓库运行时用公开读取克隆成功、但推送被 403 拒绝（`github.token` 只对自身仓库有写权限），工作流因此长期失败；现改为 `${{ github.repository }}.wiki.git`，并在 Wiki 尚未初始化（克隆报 `Repository not found`）时输出告警并跳过，其它克隆错误仍然失败且输出会脱掉 URL 中的令牌。`README.md` 与 `README.zh-CN.md` 开头补充"二改自 Christ9038/Ydisks-Xianyu-Helper"的来源与版权说明。推送前审计了工作区与全部历史：无真实密钥、无 `.env`/数据库/私钥/证书、无内外网 IP；`.gitignore` 新增忽略 `/dist/`（1.1 GB 打包产物与 Playwright runtime）、`/packaging/windows/Output/` 与 `/.zcode/`。推送后用浅克隆独立复核：1619 个文件、HEAD 与本地一致、无敏感路径。本机直连 `github.com` 不可用，推送经本地代理 `127.0.0.1:7890`（写入仓库本地配置，可随时取消），凭据经用户选择浏览器授权后由 Git Credential Manager 保存。未修改 HTTP 契约、数据库 schema、冻结 CAPTCHA、六阶段状态或注释基线；未调用真实账号或平台交易接口。

- 2026-09-23（会话界面人工接管按钮可见性修复与 Windows 安装包重建，本地未发布）：聊天会话顶栏的“接管 AI 回复”按钮在真实页面中不可见，根因有两点。一，按钮沿用深色顶栏配色 `bg-white/15 + text-white`，而当前顶栏背景已是白色，形成白底白字；现改为浅蓝徽标样式（`border-sky-200 bg-sky-50 text-sky-700`），错误提示改为 `text-red-600`。二，`ChatMetadataFeature` 的绝对定位叠加层（`absolute left-40 right-[5.5rem] top-2 z-10`）覆盖按钮命中区域，按钮容器提升为 `relative z-20`。该按钮按买家隔离：买家标识由服务端通过会话反查 `buyer_id`，前端不传买家标识；支持 5/10/15 分钟与提前结束。验证：`npm run typecheck`、`npm run comments:check`、前端 98 个测试文件共 572 项测试全部通过；重建嵌入式前端后，用真实浏览器（127.0.0.1，临时 SQLite 与临时验证会话）确认按钮位于会话顶栏、命中测试的最上层元素即按钮本身、菜单可展开 5/10/15 分钟、接管后倒计时生效、`GET /api/v1/chat/human-handoff` 返回 `buyer_id=verify-buyer` 与剩余秒数、提前结束立即清除；临时验证工具与临时数据已删除。重新构建 `dist/windows/xianyu-server.exe`（SHA-256 `2C5A638B6919E396328BE7F7814A24D5A878AA1BA68F2BB0E125A98CE86821CE`）与 Inno Setup 安装包 `dist/windows/Ydisks-Xianyu-Helper-Setup.exe`（SHA-256 `58817BFCFB25A8DF925B360CAB60B9923B211D21763F3F2AAF468489654427D9`，版本 `0.0.0-upstream`，未签名），服务参数仍为 `-workdir %ProgramData%\YdisksXianyuHelper`、`-data-key-file ...\data-key`、`-addr 0.0.0.0:59188`，数据目录不在 `{app}` 内，升级保留原数据库与密钥。未修改 HTTP 契约、数据库 schema、冻结 CAPTCHA、六阶段状态或注释基线；未调用真实账号或平台交易接口。

- 2026-09-23（对齐上游四次提交的交易安全修复，本地未发布）：按上游 `1672791`、`34eda75`、`c2c41f2`、`42a476f` 四个提交在本地实现等价修复。一，WebSocket 同步帧按 data 数组逐条提取与解密（`extractSyncPayloads` / `syncPayloadEntry`），单条解析失败只记 `decrypt_failed` 并跳过，不再因坏条目丢弃同帧后续付款事件，且每个原始帧只回一次 ACK。二，订单侧新增买卖角色门禁（`order_role_gate.go`）：WS 交易事件角色冲突或缺失时 fail-closed，角色未知必须由本地商品/订单事实补证，首次未知事件延迟到订单与商品同步后再判定，历史断点恢复同样重新核验；`db.Items.ExistsByCookieItem` 提供含软删除商品的归属查询。三，自动回复归属只看本地商品（`replyLocalItemOwnership` / `Adapter.ItemBelongsToAccount`），删除基于平台的发布人查询、`platform_unresolved` 占位与 `platform_verified` 写回，适配器未实现该能力时不再自动回复。四，订单批量同步只走售出列表，不再扫描本地订单做详情补全，`mtop` 商品详情探测（`FetchItemPublisher`、`DetectItemMultiSpec`、`ItemDetailAPI`）整体退场，刷新摘要中的详情字段标记为旧契约兼容并固定为零。五，账号级付款规则必须由用户在 `config_json` 显式开启 `allow_all_items` 才可忽略订单规格（`ruleAllowsAllItems` 冻结进运行快照），未授权时空规格动作不匹配带规格订单。六，关键词规则支持多商品（`item_ids` 契约、`SplitItemIDs`/`JoinItemIDs`、`ItemMultiSelect` 与 Toast），出站回复改为应用层 `SendReply` 端口并按图后文顺序投递。同步拆分 `internal/engine/dispatch_images.go`、`internal/adapter/token_captcha_solve.go` 以满足超大文件与超长函数门禁。`go test ./... -count=1`、`go vet ./...`、`go build ./...`、`go run ./tools/architecturecheck`、`go run ./tools/commentlint`、`npm run api:check/typecheck/comments:check`、572 项前端测试均通过；全量并发下 `internal/adapter` 的 `TestOnPasswordLoginRefreshConcurrentCallersShareResult` 出现一次计时敏感偶发失败，单独与整包重复运行均稳定通过。既有环境例外：`internal/browser` 跨盘符 `filepath.Rel` 与 `internal/server` `safeZipPath` 反斜杠断言在 Windows 上失败，均为平台相关既有断言，未为通过而改写生产逻辑。未修改冻结 CAPTCHA 行为、六阶段状态或注释基线；未调用真实账号或平台交易接口。
