...
| 4. Server 应用服务 | 进行中 | 订单、发布、登录、聊天纵向抽取 | 订单应用服务切片开始，目标是让 handler 不再直接编排基础设施 |
...
- 阶段 4 下一 PR 切片为“订单应用服务边界提取”：围绕订单列表、详情、更新、删除、刷新、导入和手动发货提取不依赖 `net/http` 的应用服务，保留 DTO/权限契约和事务不变量，不拆分单个 handler 提交；
