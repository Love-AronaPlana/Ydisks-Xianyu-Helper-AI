...
| 5. 应用生命周期装配 | 进行中 | 消除必需依赖 setter 回填 | 构造验证与幂等关闭测试 |
...
- 当前阶段：阶段 5“应用生命周期装配”（进行中）；阶段 4 Server 应用服务与阶段 3 HTTP API 契约已完成最终审计；
...
- 阶段 5 下一 PR 切片为“应用构造依赖验证与生命周期接口”：围绕 Server/应用服务构造参数、必需依赖校验、幂等 Start/Stop 和 worker 等待建立显式生命周期入口，不把 setter 替换拆成多个提交；
