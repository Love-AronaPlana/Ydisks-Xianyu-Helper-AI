...
本节记录账号级验证码处理模式 `captcha_browser_mode` 中除自动路径之外的另一条路径。它由用户在账号
设置中显式选择，与本文其余条款并列而非替代：默认值 `playwright` 继续表示本文第 3 至 8 节的自动路径，
只有账号显式保存 `system_manual` 时才启用本节流程。

这是独立的附加流程：它不执行任何自动拖动，不修改选择器、轨迹、时序、重试次数、失败恢复、
Cookie 合并和成功判定。本节只改变“由谁完成拖动”，不改变本文其他冻结行为；两者冲突时，本文其余
冻结条款继续优先适用。

选择 `system_manual` 后，该账号的本次验证按以下约束执行：

1. 自动路径不再参与：不调用远程滑块服务，也不启动 Playwright 主引擎或直接 Chromium/CDP 备用引擎。
   自动路径的失败不得被转写为人工路径的成功，人工路径失败也不得回退为自动拖动。
2. 服务只在用户已登录的交互式桌面会话中启动系统 Chrome/Edge，用该账号专用的独立浏览器 profile 打开
   本次需要验证的完整地址；该地址与第 8 节最终失败日志中的 `verification_url` 来源相同。验证地址、
   Cookie 与其他凭证仍遵守第 1 节和第 8 节的敏感信息约束：只接受 http/https 地址，人工验证 profile
   按账号隔离且不与自动化 profile 共用，浏览器启动参数和日志不得包含 Cookie、Token 或密码。
3. 服务不向页面注入轨迹、不模拟鼠标、不点击滑块，也不改写页面。用户等待期间只按固定间隔通过回环
   调试端口读取浏览器 Cookie；等待有上限，并可被调用方 Context 取消。
4. 成功判定与第 4 节一致：必须在基线之上出现相对基线全新的、非空的 `x5sec`。基线由账号当前已持有的
   `x5sec` 与页面打开时浏览器内的现值共同构成，旧值不得判成功。只有新值被合并回原 Cookie 字符串，
   其他字段保持账号当前值；超时、用户取消、未找到系统 Chrome/Edge 或没有交互式桌面会话时必须明确失败
   并保留原 Cookie，不得伪造成功，也不得降级为自动拖动。
5. 风控日志的验证引擎标识记录为 `system_manual`，与自动路径的 `playwright`、`drissionpage` 和 `remote`
   区分，便于判断账号实际走了哪条路径。

`system_manual` 只在用户显式选择时生效。读取失败、字段缺失或取值不在 `playwright` / `system_manual`
之内时，必须继续按历史的 `playwright` 自动模式处理。没有受支持的交互式桌面路径时，该模式明确报告
不可用，而不是静默改回自动拖动。

本节不改变自动模式：`playwright` 账号的默认值、行为与本文其他条款完全不变；本节也没有授权改动
第 2 至 9 节的任何一条契约。
