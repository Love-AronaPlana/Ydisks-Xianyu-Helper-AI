...
// TestFrontendDTOFieldParserRejectsNestedProperties 验证 DTO 字段解析只比较 HTTP 响应的顶层字段。
func TestFrontendDTOFieldParserRejectsNestedProperties(t *testing.T) {
	// source 是同时包含直接字段和嵌套对象字段的最小前端响应 DTO。
	source := `export interface CategoryResponse {
  success: boolean;
  category: {
    cat_id: string;
  };
}`
	// fields、lines 分别是解析出的字段集合和接口声明行号。
	fields, lines := parseFrontendResponseFields(source)
	// categoryFields 是 CategoryResponse 的顶层字段集合。
	categoryFields := fields["CategoryResponse"]
	// exists 表示 success 是否被正确识别为顶层字段。
	if _, exists := categoryFields["success"]; !exists {
		t.Fatalf("顶层 success 字段未被识别: %v", sortedFieldNames(categoryFields))
	}
	// exists 表示 category 是否被正确识别为顶层字段。
	if _, exists := categoryFields["category"]; !exists {
		t.Fatalf("顶层 category 字段未被识别: %v", sortedFieldNames(categoryFields))
	}
	// exists 表示嵌套 cat_id 是否被错误提升为顶层字段。
	if _, exists := categoryFields["cat_id"]; exists {
		t.Fatalf("嵌套 cat_id 不应被视为外层 DTO 字段: %v", sortedFieldNames(categoryFields))
	}
	if lines["CategoryResponse"] != 1 {
		t.Fatalf("接口声明行=%d want=1", lines["CategoryResponse"])
	}
}

// TestFrontendDTOFieldContractReportsMissingBackendField 验证真实 DTO 夹具缺少后端字段时字段门禁会失败。
func TestFrontendDTOFieldContractReportsMissingBackendField(t *testing.T) {
	// root 是本测试隔离出的最小仓库根目录。
	root := t.TempDir()
	// frontendDir、serverDir 分别是前端传输契约和服务端 DTO 的夹具目录。
	frontendDir := filepath.Join(root, "frontend", "shared", "api-contract")
	// serverDir 是服务端 DTO 夹具目录。
	...
	// frontendSource 是要求 required 字段的最小前端 DTO。
	frontendSource := []byte("export interface ExampleResponse {\n  required: string;\n}\n")
	// err 是写入前端 DTO 夹具失败的原因。
	if err := os.WriteFile(filepath.Join(frontendDir, "transport.ts"), frontendSource, 0o600); err != nil {
		t.Fatal(err)
	}
	// serverSource 故意不提供 required 字段，用来验证真实扫描链路。
	serverSource := []byte("package server\ntype exampleResponse struct { Provided string `json:\"provided\"` }\n")
	// err 是写入服务端 DTO 夹具失败的原因。
	if err := os.WriteFile(filepath.Join(serverDir, "response.go"), serverSource, 0o600); err != nil {
		t.Fatal(err)
	}
	// originalSpecs 保存测试前的全局登记表，结束后必须恢复避免污染其他门禁测试。
	originalSpecs := frontendDTOContractSpecs
	frontendDTOContractSpecs = []frontendDTOContractSpec{{frontendType: "ExampleResponse", backendType: "exampleResponse"}}
	defer func() { frontendDTOContractSpecs = originalSpecs }()
	// violations 是执行真实字段门禁后得到的违规集合。
	violations := checkFrontendDTOFieldContracts(root)
	if len(violations) != 1 {
		t.Fatalf("violations=%+v", violations)
	}
	// message 是字段缺失诊断，应稳定指明双方 DTO 和遗漏字段。
	message := violations[0].message
	if !strings.Contains(message, "ExampleResponse") || !strings.Contains(message, "required") || !strings.Contains(message, "exampleResponse") {
		t.Fatalf("缺失字段诊断不完整: %s", message)
	}
}

// TestServerDTOFieldParserIncludesEmbeddedDTO 验证匿名嵌入 DTO 提升的 JSON 字段不会被字段门禁误判遗漏。
func TestServerDTOFieldParserIncludesEmbeddedDTO(t *testing.T) {
	// root 是本测试隔离出的最小仓库根目录。
	root := t.TempDir()
	// serverDir 是服务端 DTO 夹具目录。
	serverDir := filepath.Join(root, "internal", "server")
	// err 是创建服务端匿名嵌入夹具目录失败的原因。
	if err := os.MkdirAll(serverDir, 0o755); err != nil {
		t.Fatal(err)
	}
	// source 使用 Go 的匿名嵌入模拟 orderDetailResponse 兼容顶层字段行为。
	source := []byte("package server\ntype baseDTO struct { ID string `json:\"id\"` }\ntype detailResponse struct { baseDTO; Success bool `json:\"success\"` }\n")
	// err 是写入匿名嵌入 DTO 夹具失败的原因。
	if err := os.WriteFile(filepath.Join(serverDir, "response.go"), source, 0o600); err != nil {
		t.Fatal(err)
	}
	// fields、_、err 分别是解析出的 DTO 字段、未使用的行号和解析错误。
	fields, _, err := parseServerResponseFields(root)
	if err != nil {
		t.Fatal(err)
	}
	// detailFields 是含匿名嵌入字段的详情 DTO 字段集合。
	detailFields := fields["detailResponse"]
	// exists 表示匿名嵌入的 id 是否已提升。
	if _, exists := detailFields["id"]; !exists {
		t.Fatalf("匿名嵌入 id 字段未被解析: %v", sortedFieldNames(detailFields))
	}
	// exists 表示详情 DTO 的直接 success 字段是否已保留。
	if _, exists := detailFields["success"]; !exists {
		t.Fatalf("直接 success 字段未被解析: %v", sortedFieldNames(detailFields))
	}
}
