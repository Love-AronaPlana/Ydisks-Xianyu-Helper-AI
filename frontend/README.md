...
`-addr :59188` 会监听全部网络接口；本机开发以外应显式使用回环地址或配置网络访问控制。桌面安装包按平台区分监听地址：Windows 服务监听 `0.0.0.0:59188`，可用 `http://<主机IP>:59188` 访问，本机仍可用 `http://127.0.0.1:59188`；macOS 和 Linux 安装包保持回环地址 `http://127.0.0.1:59188`。Windows 管理端口对局域网开放时必须由 Windows 防火墙或等价网络 ACL 限制来源，不得暴露到公网。
