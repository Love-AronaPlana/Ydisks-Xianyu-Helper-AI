...
export const mergeAccountAISettings = (
  ...
  account => ({
    ...
    ai_reply_mode: allAISettings[account.id]?.ai_reply_mode ?? 'bargain',
    ai_full_prompt: allAISettings[account.id]?.ai_full_prompt ?? '',
    human_handoff_minutes: allAISettings[account.id]?.human_handoff_minutes ?? 0,
    ai_vision_enabled: allAISettings[account.id]?.ai_vision_enabled ?? true,
