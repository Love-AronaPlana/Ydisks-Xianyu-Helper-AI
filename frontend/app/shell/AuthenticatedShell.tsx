...
// ItemAIPromptModal 在商品页面中按需加载，避免商品级提示词与变量编辑逻辑增大商品主分片。
const ItemAIPromptModal = lazy(/* 商品 AI 提示词编辑器随商品页面路由加载。 */ () => import('../features/items/components/ItemAIPromptModal').then(/* module 提供提示词编辑器的具名导出。 */ module => ({ default: module.ItemAIPromptModal })));
...
export const AppContent: React.FC<AppContentProps> = ({
  ...
  const renderPage = () => {
    switch (activeTab) {
      ...
      case 'items': return <ItemList onConfigureDelivery={handleConfigureDelivery} publishSpecsEditor={PublishSpecsEditor} publishImagesEditor={PublishImagesEditor} itemAIPromptEditor={ItemAIPromptModal} />;
